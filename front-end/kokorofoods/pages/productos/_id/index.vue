        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <p>
                Al parecer estas intentando agregar un producto de otra tienda
                que no corresponde a tu pais
              </p>
      ...
      <div class="row featurette">
        <div class="col-md-7 order-md-2 px-5">
          ...
          <div class="row info-block">
            <!--row2-->
            <div
              class="col-md-12 row py-5"
              v-html="this.$t('ProductPage.Benefits')"
            ></div>
          </div>
        </div>
        <div class="col-md-5 order-md-1">
          <p>
            <NuxtLink
              ...
              >{{ $t("Buttons.Back") }}</NuxtLink
            >
          </p>
          <h2 class="featurette-heading pr-3 py-3 pl-0">
            {{ ProductData.name }}
          </h2>
          <p class="precio">
            <strong>
              {{ $t("general.price") }}:
              <span>{{ $n(ProductData.price, "currency") }}</span></strong
            >
          </p>
          <div
          ...
          >
            <div class="carousel-inner">
              <div
                class="carousel-item single-art"
                v-for="(img, index) in ProductImage"
                :key="img.id"
                :class="['slide' + index, { active: ActiveCalc(index) }]"
              >
                <img
                  :src="img.image_url"
                  class="d-block w-50"
    ...
    <section class="recetas container col-md-12 mt-5 p-0" v-if="mobile">
      <div class="row featurette p-0">
        <div class="col-12 p-0">
          <h2 class="featurette-heading pt-5 pl-4 pr-5">
            {{ ProductData.name }}
          </h2>
          <p class="precio pl-4 pr-5">
            {{ $t("ProductPage.price") }}:
            <span>${{ $n(ProductData.price, "currency") }}</span>
          </p>
          ...
          <div class="col-12 p-0 info-articulo pt-3">
            <p v-if="ProductData.quantity > 0">
              <button
                class="btn btn-lg btn-light center-button-2 mt-3 mb-3"
                @click="
                  addProductToCart({
                    product: ProductData,
                    quantity: 1,
            ...
            <p v-if="ProductData.quantity < 1">
              <button
                class="btn btn-lg btn-light center-button-2 mt-3 mb-3 disabled"
              >
                {{ $t("Buttons.addtocart") }}
              </button>
            </p>
            <div
              class="n-articulo-lg pl-4 pr-4"
              v-html="ProductData.description"
            ></div>
            <p class="n-articulo-lg pl-4 pr-4 pb-5"></p>
          </div>

          <div class="row info-block mt-5 mb-5">
            <!--row1-->
          ...
          <div class="row info-block">
            <!--row2-->
            <div
              class="col-12 row p-4"
              v-html="this.$t('ProductPage.BenefitsMobile')"
            ></div>
          </div>

          <p>
            <NuxtLink
              class="btn btn-lg btn-primary center-button-2 mt-3 mb-3"
              :to="localePath('/productos')"
              >{{ $t("Buttons.Back") }}</NuxtLink
            >
          </p>
      ...
      meta: [
        {
          hid: "description",
          name: "description",
          content: this.ProductData.description,
        },
        {
          hid: "og:title",
          name: "og:title",
          content: this.ProductData.name,
        },
        {
          hid: "og:description",
          name: "og:description",
